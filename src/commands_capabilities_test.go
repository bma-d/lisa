...
func TestCommandCapabilitiesCriticalFlagContracts(t *testing.T) {
	required := map[string][]string{
		"session monitor":        {"--until-jsonpath", "--event-budget", "--webhook", "--timeout-seconds", "--auto-recover", "--adaptive-poll"},
		"session explain":        {"--since"},
		"session context-pack":   {"--from-handoff", "--redact"},
		"session route":          {"--budget", "--topology", "--cost-estimate", "--profile", "--queue", "--concurrency", "--strict"},
		"session packet":         {"--fields", "--delta-json"},
		"session capture":        {"--semantic-delta", "--strip-banner"},
		"session smoke":          {"--chaos-report", "--export-artifacts", "--llm-profile", "--contract-profile"},
		...
		"session budget-observe": {"--from", "--from-jsonl", "--tokens"},
		"session budget-enforce": {"--max-tokens", "--from", "--from-jsonl"},
		...
		"session state-sandbox":  {"--action", "--file"},
		"session turn":           {"--session", "--text", "--keys", "--until-jsonpath"},
