...
- **18:02:20**: Fix codex exec nested flags + tmux diagnostics
  Codex exec now auto-adds --skip-git-repo-check, omits --full-auto when bypass sandbox flag present, and rejects bypass+full-auto conflict. Tmux command errors now include stderr/stdout text for actionable spawn failures.
- **18:02:21**: Refresh context docs for waiting_input and nested codex guidance
  Updated USAGE.md, .agents/architecture.md, and .agents/sdk-usage.md to reflect emitted waiting_input state and codex nested execution flag behavior.
