...
- **16:48:41** (`src/session_wrapper.go`): Unset nested Claude env in wrapper
  Added explicit 'unset CLAUDECODE' before wrapped session commands so Claude CLI does not reject nested execution inside lisa tmux sessions.
- **16:48:41** (`src/regressions_test.go`): Add nested-env regression coverage
  Extended wrapper/fallback tests to assert CLAUDECODE is cleared in generated scripts.
- **16:48:41** (`src/tmux.go`): Unset nested Claude env in fallback script
  Added 'unset CLAUDECODE' at fallback script prologue to prevent nested-session guard when respawning pane commands.
