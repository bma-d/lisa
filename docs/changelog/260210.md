...
## 260210-22:25:45 - Harden event observability retention and session cleanup

### Summary
Reduced event-log contention risk, added stale-event retention pruning, and closed remaining cleanup/observability gaps with regression coverage.

### Added
- New regression suite `src/stability_findings_test.go` covering pane-root PID process matching, cross-hash kill-all cleanup semantics, spawn failure lifecycle events, event-tail lock timeout behavior, large-line event-tail decoding, and stale event-log retention pruning.
- Configurable stale event artifact retention via `LISA_EVENT_RETENTION_DAYS` (default `14`).

### Changed
- Agent process detection now considers the pane root PID process in addition to descendants.
- `session kill-all` now enables cross-hash artifact cleanup by default when not using `--project-only`.
- Spawn failure paths now emit lifecycle failure reasons (`spawn_*_error`) for observability parity.
- Event-tail reads now use shared file locking and reader-based line parsing to avoid scanner token-size limits.
- Event-log trimming now compacts from a bounded tail window to cap trim work even for large historical logs.
- Spawn/kill/kill-all now run stale event artifact retention cleanup as maintenance.

### Fixed
- Prevented missed agent detection when tmux pane PID is the agent process itself.
- Prevented cross-project artifact leaks after global `kill-all` runs.
- Prevented observability blind spots where spawn failures only surfaced on stderr.
- Reduced risk of explain-path read failures on very large JSONL event lines.
...
### QA Notes
- `go test ./src -run 'TestDetectAgentProcessMatchesPaneRootProcess|TestCmdSessionKillAllDefaultAlsoCleansCrossHashArtifacts|TestCmdSessionKillAllProjectOnlyKeepsCrossHashArtifactsByDefault|TestCmdSessionSpawnFailureEmitsLifecycleReason|TestReadSessionEventTailRespectsWriterLockTimeout|TestReadSessionEventTailHandlesVeryLargeLine|TestPruneStaleSessionEventArtifactsRemovesOldLogs'`
- `go test ./...`
- `go test ./... -race`
- `go vet ./...`
- `go test ./... -coverprofile=/tmp/lisa.cover.out`
