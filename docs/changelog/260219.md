...
## 260219-21:59:10 - Resolve codex exec nested flag conflicts and improve tmux spawn diagnostics

### Summary

Hardened codex exec command assembly for nested Lisa runs, improved tmux failure visibility, and synced docs to emitted waiting-input behavior.

### Added

- Added regression tests for codex exec default flags, bypass behavior, and bypass/full-auto conflict rejection.
- Added coverage for tmux error wrapping that includes tmux stderr/stdout context.

### Changed

- Changed codex exec command generation to always include `--skip-git-repo-check` by default.
- Changed bypass sandbox handling to omit default `--full-auto` when `--dangerously-bypass-approvals-and-sandbox` is present.
- Updated spawn hint/help/usage/context docs for nested codex guidance and waiting-input semantics.

### Fixed

- Fixed codex CLI invocation failures caused by passing both `--full-auto` and `--dangerously-bypass-approvals-and-sandbox` together.
- Fixed opaque tmux spawn errors by surfacing tmux command output in returned errors.
...
- Verify `./lisa agent build-cmd --agent codex --mode exec --prompt hi` returns `--full-auto` and `--skip-git-repo-check`.
- Verify bypass mode omits `--full-auto`: `./lisa agent build-cmd --agent codex --mode exec --prompt hi --agent-args '--dangerously-bypass-approvals-and-sandbox'`.
- Verify bypass+full-auto fails fast with validation error.
- Verify nested parent codex exec with bypass can spawn/monitor/capture a child Lisa session successfully.
- Verify spawn tmux failures now include tmux stderr text for diagnostics.
- Verify `go test ./...` and `./smoke-nested --max-polls 120` pass.
