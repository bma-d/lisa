...
## 260222-22:44:12 - Surface explicit session socket paths for cross-socket tooling

### Summary
Persisted and exposed per-session tmux socket paths so external tools can discover Lisa sessions without socket guessing.

### Added
- Added `socketPath` to persisted session metadata.
- Added `socketPath` to `session spawn --json` success payload.
- Added `socketPath` to `session list --with-next-action --json` item payloads.

### Changed
- Changed `session list --all-sockets` probing to use metadata `socketPath` when available.
- Changed session-list delta equality to include socket-path changes.

### Fixed
- Fixed cross-socket discovery blind spots when session socket naming/location differs from derived project-hash defaults.
...
### QA Notes
- Verify `./lisa session spawn --agent codex --mode exec --prompt "echo hi" --json` includes `socketPath`.
- Verify `./lisa session list --all-sockets --with-next-action --json` includes `items[].socketPath`.
- Verify sessions with custom socket metadata are still detected by `session list --all-sockets`.
