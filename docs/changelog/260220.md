...
## 260220-15:27:22 - Harden cross-socket regression test hermeticity

### Summary
Removed real tmux dependency from cross-socket list regression coverage by stubbing list calls in test setup.

### Fixed

- Fixed `TestCmdSessionListAllSocketsFindsCrossRootSessions` to stub `tmuxListSessionsFn`, preventing environment-dependent failures on hosts without tmux/socket availability.
...
- Verify hermetic behavior by running `go test ./src -run TestCmdSessionListAllSocketsFindsCrossRootSessions -count=1` on a machine without active tmux server.
- Verify full suite still passes: `go test ./...`.


## 260220-16:56:29 - Add skills install/sync flows and release skill gate

### Summary
Introduced lisa skills sync/install plus a pre-release checklist so tagged builds fetch skills from GitHub and dev builds use repo skills.

### Added
- Added `lisa skills sync` and `lisa skills install` commands for skill propagation across codex/claude/project targets.
- Added recursive copy + path resolution helpers for skills management and command coverage tests.
- Added release-aware skills source resolution: dev builds use repo `skills/lisa`; tagged builds fetch `skills/lisa/SKILL.md` from GitHub refs.
- Added pre-release checklist doc at `docs/pre-release-skills-checklist.md`.

### Changed
- Updated CLI help, usage docs, README, and context index docs for new skills workflows.
- Updated project context metadata to include `skills {sync,install}` command surface and vendored `skills/lisa` tree.

### Fixed
- Fixed release-install fetch robustness with multi-ref resolution and explicit HTTP timeout for GitHub skill fetch.
...
### QA Notes
- Verify dev binary path uses repo skill: `./lisa skills install --to project --project-path <tmp> --repo-root "$(pwd)"`.
- Verify release path attempts GitHub fetch by tag/ref using a tagged build (expected to require remote `skills/lisa/SKILL.md` in published ref).
- Verify command help and tests: `./lisa skills --help`, `./lisa skills sync --help`, `./lisa skills install --help`, `go test ./...`.

## 260220-18:00:53 - Default skills install auto-targets codex and claude

### Summary
Updated lisa skills install defaults to install into all available ~/.codex and ~/.claude targets when --to/--path are omitted.

### Added
- Added default auto-target discovery for `~/.codex` and `~/.claude` when running `lisa skills install` without `--to`/`--path`.
- Added multi-target install JSON payload support via `{"installs":[...]}` for default auto mode.
- Added regression tests for default dual-target install and no-target error behavior.

### Changed
- Changed `skills install` default behavior from single-target codex install to auto-install across available codex/claude homes.
- Updated CLI help text and usage docs to reflect auto-target defaults and override semantics.
...
### QA Notes
- Verify default auto mode installs to both targets when available: `HOME=<tmp> ./lisa skills install --repo-root "$(pwd)" --json`.
- Verify explicit override still works: `./lisa skills install --to project --project-path <tmp> --repo-root "$(pwd)" --json`.
- Verify validation path when no defaults exist returns clear error; verify suite remains green: `go test ./...`.
