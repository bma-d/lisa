...
Commands: `doctor` | `capabilities` | `oauth {add,list,remove}` | `session {name,spawn,send,status,explain,monitor,capture,tree,smoke,list,exists,kill,kill-all}` | `agent build-cmd` | `skills {sync,install}`
